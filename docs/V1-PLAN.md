...
| `src/ui.js` | 设置抽屉（Relay 地址、连接按钮、状态显示；联机名称自动取当前 Persona） | 房间面板全部（入房、时间线、提案、审核、副聊天） |
...
| `auth.hello` | 所有人 | 连接后上报当前酒馆 Persona 名称与恢复凭据；联机中切换 Persona 时复用本命令更新名称 |
| `room.create` | 房主 | 建房 |
| `room.join` | 客人 | 凭邀请码入房 |
...
| `room.member.online` | `{ clientId, displayName? }` | 成员断线重连或在线更新 Persona；有 `displayName` 时客户端同步刷新成员名 |
...
- [x] 镜像聊天（2026-07-12 直连化实现，代码完成）：镜像 = 导入的共享卡 + 导入的联机存档聊天；客机切到该聊天即自动绑定（`CHAT_CHANGED` 钩子），时间线事件写成署名消息（`extra.stmpMessageId` 幂等标记），AI 回复写成角色消息；离开期间落下的故事在切回时 `catchUp` 补写。`force_avatar` 头像对齐与 seq 错位尾部重写留待后续打磨。
- [x] 输入框接管（直连模式，无审核）：客机在镜像聊天中经原生输入框发送 → `MESSAGE_SENT` 钩子直接发布 `story.message.publish`（消息就地保留，回声事件按文本反查补同步标记，不二次写入）；本地生成由 manifest `generate_interceptor` 在镜像角色上 `abort(true)` 中止。房主同理经原生输入框发言，发送后的自动生成即"收束回合"。
- [x] 流式镜像：房主端 `GENERATION_STARTED/STREAM_TOKEN_RECEIVED/GENERATION_ENDED` 钩子把原生触发的生成全程转发（300ms 节流全文快照）；客机端在镜像聊天里渲染实时刷新的流式气泡，权威 assistant 消息到达后原地定稿，生成失败则移除气泡。
- [ ] 只读保护：编辑/删除/swipe 一律回滚并提示"联机镜像为只读"。（V1 暂缓：镜像是客机本地副本，发散可由联机存档重新同步修复）
- [ ] **卡片同步两档**：
  - 档位一（默认开）：房主同步角色名字 + 头像 → 客人端自动创建"皮肤卡"（定义为空），镜像挂其下——观感与本地卡一致，人设定义不离开房主机器；（2026-07-12 注：当前实现直接走档位二完整卡，档位一降级为可选后续）
  - [x] 档位二（房主**每房间显式开启**，默认关）：完整卡 PNG 经资产通道传输，客人端按内容哈希命名覆盖导入并选中（跨房间去重复用）；开启时确认框提示卡片可被成员保存、外部挂载世界书不随卡走。支持更新与停止共享，停止后 Relay 立即删除资产。（真机 UI 验收仍并入上方第 5 项）
- [ ] 散场留存：房间结束后提供"解除镜像锁定"与"**复制为可玩副本**"（镜像卡与存档聊天本就是客机本地文件，房间结束后天然可继续单机玩；显式的"复制副本"入口留待打磨）。
- [x] 协议增量：`room.card.update/clear`、`room.chat.update/clear`、`round.ready` 与对应事件已同步插件与 Relay，并纳入 resume 日志和 smoke。
- [ ] **验收**：客人在原生聊天界面完成一局全流程（原生输入发送 → 全员实时可见 → 房主收束 → 流式输出出现在镜像聊天 → 权威回复定稿），观感与本地卡一致；散场后单机续玩成功；客人自有聊天与角色数据全程零改动。（代码完成于 2026-07-12，待真机双人手测）

## 4. 插件侧约束（评审逐条检查）

1. 酒馆 API 只走 `getContext()`（P2 铁律，全模块适用）。
2. 永不触碰客人**自有**的聊天、角色与设置数据；联机写入仅限插件自建并托管的镜像聊天与皮肤卡/导入卡（P3），且全部可整体删除、不留残余。
3. 扩展设置里只存 `relayUrl`、`reconnect`、恢复凭据和本地共享资源去重记录；联机显示名与故事署名每次读取当前 Persona，不另存昵称；不存 API key、不缓存他人隐私内容。
