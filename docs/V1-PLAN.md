...
- [ ] `ui.js` 房间面板：设置抽屉里只保留连接配置；房间界面（入房后）做成**可拖动、可调大小的浮动窗口**，时间线用聊天气泡样式，消息渲染借用 `getContext()` 暴露的酒馆 markdown 格式化工具，观感对齐原生聊天：
  - [ ] 未入房：粘贴邀请码入房 / 房主建房 + 生成邀请码（一键复制）；
  - [ ] 已入房（所有人）：成员列表（含在线状态）、故事时间线视图、副聊天、"生成中"指示；
  - [ ] 客人：提案编辑器（提交/撤回、显示自己提案的审核状态）；
  - [ ] 房主：提案审核队列（接受/拒绝按钮）。
  - 注：客人自有的原生聊天永不被触碰。P1 阶段客人视图是**独立面板的只读投影**；原生观感的故事界面由 P3 的插件托管镜像聊天承担，届时本面板退化为控制中心并保留时间线视图作回退。
...
### P3 — 镜像聊天模式（V1 冲刺项，2026-07-11 决定）

> 目标：客人的故事主界面复用原生聊天渲染（头像气泡、主题、TTS/翻译等扩展全部生效），同时保留房主权威模型不变。前置条件：P1、P2 完成，relay M4.5 资产通道可用。

- [ ] **技术验证（开工门槛，半天）**：五项全过才开工，任一不过则镜像降级为 V2、客人显示回退 P1 面板：
  1. 消息截获——发送类事件能取出刚发送的消息并从聊天数组移除；
  2. 生成拦截——能阻止客人在镜像聊天中触发 AI 生成；
  3. 编辑回滚——编辑/删除/swipe 事件可监听并恢复原状；
  4. 卡片导入/导出可经程序调用（PNG 含嵌入定义）；
  5. 资产通道端到端（房主导出 → 中继 → 客人导入）。
